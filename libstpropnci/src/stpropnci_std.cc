...
static bool stpropnci_cb_core_reset_rsp(
      ...
      switch (oid) {
              ...
              *buflen = payloadlen;
              ...
              if (idx_sc != 0) {
                ...
                if ((route_sc & 0x80) == 0x80) {
                  ...
                  if ((route_sc_ps == 0x00) && (stpropnci_state.is_ese_felica_enabled)) {
                    // Change the system code power state to 0x3B
                    LOG_D("Overwrite power state for default system code route");
                    pp[idx_sc + 3] = 0x3B;
                  }
                }
              }
          ...
          if (mt == NCI_MT_CMD) {
            ...
            if (stpropnci_state.ts_last_rf_deactivate.tv_sec != 0) {
              ...
              if ((now.tv_sec < stpropnci_state.ts_last_rf_deactivate.tv_sec) ||
                  ((now.tv_sec == stpropnci_state.ts_last_rf_deactivate.tv_sec) &&
                   (now.tv_nsec < stpropnci_state.ts_last_rf_deactivate.tv_nsec))) {
                struct timespec req;
                req.tv_sec = stpropnci_state.ts_last_rf_deactivate.tv_sec -
                             now.tv_sec;
                if (stpropnci_state.ts_last_rf_deactivate.tv_nsec >=
                    now.tv_nsec) {
                  ...
                  req.tv_sec--;
                  req.tv_nsec = 1000000000LL + stpropnci_state.ts_last_rf_deactivate.tv_nsec -
                                now.tv_nsec;
                }
        ...
        case NCI_MSG_RF_DEACTIVATE:
          if (mt == NCI_MT_CMD) {
            ...
            (void)clock_gettime(CLOCK_MONOTONIC, &stpropnci_state.ts_last_rf_deactivate);
...
bool stpropnci_send_core_reset_ntf_recovery(uint8_t hint) {
  ...
  stpropnci_state.nfc_recovery_hint = hint;
  stpropnci_state.nfc_recovery = true;

  // Send CORE_RESET_CMD to NFCC
...
/*******************************************************************************
**
** Function         stpropnci_cb_core_reset_rsp
**
** Description      CORE_RESET_RSP, send NTF to stack
**
**
...
static bool stpropnci_cb_core_reset_rsp(
  ...
  // trigger 0.
  UINT8_TO_STREAM(pp, stpropnci_state.nfc_recovery_hint >= 0xA0
                          ? stpropnci_state.nfc_recovery_hint
                          : 0x00);
