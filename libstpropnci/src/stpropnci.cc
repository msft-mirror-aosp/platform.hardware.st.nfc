...
#define VARIANT "vendor"
  ...
  std::string felicaValue = stpropnci_cfg_read_value(STPROPNCI_CFG_FELICA_ESE_SUPPORT);
...
void stpropnci_change_config(const char * config_key, const char * config_value) {
  /* Behavior depends on the config_key being updated, this only accepts known configs */
